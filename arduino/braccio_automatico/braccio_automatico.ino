...
const float downCoordinates = 0.25, upCoordinates = 20.0;//[mm]
const float mSecPerDeg = 2.666666667;//velocità motore in [deg/msec]

bool penUp = false, finish = false;
...
float angleTarget[4],//[rad], angolo che devono avere motori
      ...
      defaultCoordinates[3] = {60,0,upCoordinates},
      coordinates[3] = {60,0,downCoordinates}, //coordinate end effector xyz
      tempCoordinates[3];
int   commands[20];
...
void loop() {
  ...
  if(Serial.available()==1){
    ...
    while(iter<nCommands){
      for(int i=0;i<3;i++) tempCoordinates[i] = coordinates[i];
      if(commands[iter] == 254) {/*
        penUp = true;
        tempCoordinates[2] = upCoordinates;
        checkCoordinates();
        MOVE();*/
        ...
        penUp = true;
        for(int i=0;i<3;i++) tempCoordinates[i] = defaultCoordinates[i];
        checkCoordinates();
        MOVE();
      }
      else{
        tempCoordinates[0] = commands[iter];
        iter++;
        tempCoordinates[1] = commands[iter]-80;
        checkCoordinates();
        MOVE();/*
        if(penUp){
          penUp = false;
          tempCoordinates[2] = downCoordinates;
          checkCoordinates();
          MOVE();
        }*/
      }
