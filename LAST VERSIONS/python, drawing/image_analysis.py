    ...
    for i in range(len_edges):
        for j in range(len_edges):
            if edges[i,j] > 0:
                ...
                graph = compute_graph(edges, i, j, precision, tollerance)
                if type(graph) == type(Graph('i')): graphes.append(graph)
...
def delete(edges,cell, x, y):
    for i in range(x,x+11):
        for j in range(y,y+11):
            edges[check_bounds(i),check_bounds(j)] = 0
...
def DPS(graph):
    ...
    for i in range(len(graph.nodes)):
        if len(graph.nodes[i].get_links()) == 1:
            i_node = i
            break
    ...
    if len(path)>10: return path

def merge_graphes(vett):
    if booldebug:
        for v in vett:
            print(v[0],v[-1])
        print("start")
    ...
    while i < len(vett):#scorro vettore con while
        link = False
        for n1 in vett[i].nodes:#per nodo del grafo i
            if len(n1.get_links())==1:
                for j in range(len(vett)):#scorro gli ALTRI grafi
                    if i!=j:
                        for n2 in vett[j].nodes:#scorro nodi 2° grafo
                            if len(n2.get_links())==1 and n1.get_point().check_tollerance(n2.get_point(),10):
                                for n in vett[j].nodes: vett[i].nodes.append(n)#trasferissco grafo j in i
                                vett[i].manage_links(n1,[n2.get_point()])#creo link tra n1 e n2
                                del vett[j]
                                if booltest: print("graphs linked")
                                link = True
                                break
                        if link: break
            if link: break
...
def draw_path(path,redim):
    ...
    for p in path:
        if p == '_':
            jake.penup()
            if booltest: print("jump")
            count_j+=1
        else:
            jake.goto(p.get_x()*redim, p.get_y()*redim)
            if not jake.isdown(): jake.pendown()
    if booltest: print(count_j)
...
if __name__=="__main__":
    ...
    graphes = analisi(edges,precision = 5, tollerance = 4)
    v_graph = merge_graphes(graphes)
    for graph in v_graph:
        paths.append(DPS(graph))
    for path in paths:
        if type(path) == type([]):
            for p in path:
                vett.append(p)
